...
fn dsa_imprime_valor(value: &i32) {
    println!("Valor: {}", value);
}

// Função que incrementa o valor de uma referência mutável a um i32
fn dsa_incrementa_valor(value: &mut i32) {
    *value += 1;
}

fn main() {

    // Gerenciamento de memória com referências
    ...
    // dsa_incrementa_valor(&x); 

    // Gerenciamento de memória com referências mutáveis
    ...
    let mut y = 20;
    dsa_incrementa_valor(&mut y); 
    dsa_imprime_valor(&y); 

    // Gerenciamento de memória com Box
    ...
    dsa_imprime_valor(&boxed_value); 

    // Isso não é permitido em Rust
    // dsa_incrementa_valor(&boxed_value); 

    // Gerenciamento de memória com Rc (contagem de referência)
